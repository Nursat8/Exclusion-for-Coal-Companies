...
# ──────────────────────────────────────────────────────────────────────────────
# 1. UNIQUE COLUMNS  •  2. FUZZY RENAME  •  3. NORMALISE KEY
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. LOAD   S&P GLOBAL   (multi-row header)
# ──────────────────────────────────────────────────────────────────────────────
def load_spglobal(file, sheet_name="Sheet1"):
    try:
        ...
        if len(full_df) < 6:
            raise ValueError("SPGlobal sheet is missing header rows")
...
# ──────────────────────────────────────────────────────────────────────────────
# 5. LOAD   URGEWALD   (single-row header)
# ──────────────────────────────────────────────────────────────────────────────
...
def merge_ur_into_sp_opt(sp_df, ur_df):
    ...
    for idx, r in sp.iterrows():
        if r["norm_isin"]:
            isin_map.setdefault(r["norm_isin"], idx)
        if r["norm_lei"]:
            lei_map.setdefault(r["norm_lei"], idx)
        if r["norm_name"]:
            name_map.setdefault(r["norm_name"], idx)
    ...
    for c in ["norm_isin", "norm_lei", "norm_name"]:
        merged_sp.drop(columns=[c], inplace=True, errors="ignore")
...
# ──────────────────────────────────────────────────────────────────────────────
# 7. EXCLUSION LOGIC  (Level-1 & Level-2)
# ──────────────────────────────────────────────────────────────────────────────
def compute_exclusion(row, **params):
    """
    Returns a Series [Excluded(bool),  Exclusion Reasons(str)]
    All numeric comparisons respect params['inclusive_ge'] ( > vs ≥ ).
    """
    reasons = []

    # helper ─ choose > or ≥ according to the toggle
    def cmp(val, thr):
        return val >= thr if params["inclusive_ge"] else val > thr

    ge_sym = "≥" if params["inclusive_ge"] else ">"
    ...
    # ─── 3. screens common to all rows ────────────────────────────────────────
    if params["exclude_mt"] and "10mt" in prod_str:
        reasons.append(">10 MT indicator")
    if params["exclude_capacity"] and cmp(cap, params["capacity_threshold"]):
        reasons.append(f"Installed capacity {cap:.0f} MW {ge_sym} {params['capacity_threshold']} MW")
    if params["exclude_power_prod"] and cmp(ur_pp_pct, params["power_prod_threshold"]):
        reasons.append(f"Coal power production {ur_pp_pct:.2f}% {ge_sym} {params['power_prod_threshold']}%")

    # ─── 4. S&P rules ─────────────────────────────────────────────────────────
    if has_sp:
        if params["sp_mining_checkbox"] and cmp(sp_min, params["sp_mining_threshold"]):
            reasons.append(f"SP mining revenue {sp_min:.2f}% {ge_sym} {params['sp_mining_threshold']}%")
        if params["sp_power_checkbox"] and cmp(sp_pow, params["sp_power_threshold"]):
            reasons.append(f"SP power revenue {sp_pow:.2f}% {ge_sym} {params['sp_power_threshold']}%")
        if params["sp_level2_checkbox"]:
            combo = sp_min + sp_pow
            if cmp(combo, params["sp_level2_threshold"]):
                reasons.append(f"SP level-2 combined {combo:.2f}% {ge_sym} {params['sp_level2_threshold']}%")

    # ─── 5. Urgewald rules ────────────────────────────────────────────────────
    if has_ur:
        if is_mining_only and params["ur_mining_checkbox"] and cmp(ur_rev_pct, params["ur_mining_threshold"]):
            reasons.append(f"UR mining revenue {ur_rev_pct:.2f}% {ge_sym} {params['ur_mining_threshold']}%")
        if is_power_only and params["ur_power_checkbox"] and cmp(ur_rev_pct, params["ur_power_threshold"]):
            reasons.append(f"UR power revenue {ur_rev_pct:.2f}% {ge_sym} {params['ur_power_threshold']}%")
        if is_mixed and params["ur_mixed_checkbox"] and cmp(ur_rev_pct, params["ur_mixed_threshold"]):
            reasons.append(f"UR mixed revenue {ur_rev_pct:.2f}% {ge_sym} {params['ur_mixed_threshold']}%")
        if params["ur_level2_checkbox"] and cmp(ur_rev_pct, params["ur_level2_threshold"]):
            reasons.append(f"UR level-2 revenue {ur_rev_pct:.2f}% {ge_sym} {params['ur_level2_threshold']}%")
...
def main():
    ...
    # ── New ≥ / > toggle ──────────────────────────────────────────────────────
    st.sidebar.markdown("---")
    inclusive_ge = st.sidebar.checkbox("Use ≥ instead of > for numeric thresholds", value=False)
    ...
    with st.sidebar.expander("Mining", expanded=True):
        ...
        exclude_mt          = st.checkbox("Exclude >10 MT flag", True)
    ...
    with st.sidebar.expander("Power", expanded=True):
        ...
        exclude_power_prod  = st.checkbox("Exclude by Coal-power-production %", True)
        power_prod_threshold = st.number_input("Power production % threshold", value=20.0)
        exclude_capacity    = st.checkbox("Exclude by installed capacity (MW)", True)
        capacity_threshold  = st.number_input("Capacity threshold (MW)", value=10_000.0)
    ...
    if sp_df.empty or ur_df.empty:
        st.warning("Error loading one (or both) sheets – please check sheet names")
        st.stop()

    # ── Merge & numeric clean-up ──────────────────────────────────────────────
    ...
    st.write(f"Retained Companies (Merged): **{len(retained_merged)}**")
