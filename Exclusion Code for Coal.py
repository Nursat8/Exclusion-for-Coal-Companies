...
def fuzzy_rename_columns(df, rename_map):
    ...
    for final_name, patterns in rename_map.items():
        for col in df.columns:
            if col in used_cols:
                continue
            # For Urgewald, if renaming to "Company", skip "Parent Company"
...
##############################################
# The compute_exclusion function now checks source.
def compute_exclusion(row, **params):
    ...
    # Determine source: if SP_ENTITY_NAME exists, record is from SP; otherwise, UR.
    if str(row.get("SP_ENTITY_NAME", "")).strip():
        ...
        if ("power" in str(row.get("Coal Industry Sector", "")).lower() or "generation" in str(row.get("Coal Industry Sector", "")).lower()):
            ...
            if params["exclude_capacity"] and (installed_cap > params["capacity_threshold"]):
                reasons.append(f"Installed capacity {installed_cap:.2f}MW > {params['capacity_threshold']}MW")
        # UR thresholds for services:
        # If the record is marked as services (by sector containing "service"), apply UR services threshold.
        if "service" in str(row.get("Coal Industry Sector", "")).lower():
            if params.get("ur_services_checkbox", False) and (coal_rev * 100) > params.get("ur_services_threshold", 0):
...
def main():
    ...
    # Sidebar: Services Section (UR only)
    with st.sidebar.expander("Services", expanded=True):
        ur_services_checkbox = st.checkbox("Urgewald: Exclude if coal revenue > threshold (services)", value=False, key="ur_services")
        ur_services_threshold = st.number_input("UR Services: Max coal revenue (%)", value=10.0, key="ur_services_threshold")
    ...
    if st.sidebar.button("Run", key="run_button"):
        ...
        # Compute threshold exclusions using a unified function that checks source.
        def compute_exclusion(row, **params):
            reasons = []
            try:
                coal_rev = float(row.get("Coal Share of Revenue", 0))
        ...
        # Build output groups:
        # Excluded Companies: union of all excluded (from merged, SP unmatched, UR unmatched)
        ...
        # For excluded, process UR records similarly.
        ...
        # Final output sheets (in order):
        # 1. Excluded Companies
        # 2. Retained Companies (merged & retained)
        # 3. S&P Only (unmatched retained)
        # 4. Urgewald Only (unmatched retained)
        output = io.BytesIO()
